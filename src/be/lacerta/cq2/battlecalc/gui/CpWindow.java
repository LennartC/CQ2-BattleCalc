...
public class CpWindow extends JDialog {
...
//    private String compactRE =     "\\W*(.*)\\W+?(\\w*)/(\\w*).*,\\W+?(\\w*|kingdom defense)\\W+?L(\\d+?)\\W+?(\\d+?)/(\\d+?)\\W+?F(\\d+?)/D(\\d+?)/A(\\d+?)/E(\\d*)(\\D*)";
     ...
     */
    public CpWindow(MainFrame mg, Hashtable<String, Creature> critMap) {
      //parent = mg;
    ...
    private void initGUI() {
        ...
        inputPanel.add(new JLabel("Attacking creature: "));
        inputPanel.add(new JScrollPane(crit1));
        inputPanel.add(new JLabel("Defending creature: "));
        inputPanel.add(new JScrollPane(crit2));
        inputPanel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
        SpringUtilities.makeCompactGrid(inputPanel, 4, 1, 5, 5, 5, 5);

        disclaimer = new JTextArea();
        disclaimer.setText("  Paste the creatures in COMPACT MODE only.\n");
        disclaimer.append("  This is kinda experimental, so check if it's correct\n");
        disclaimer.append("  and don't forget to report bugs! ;-)");
        disclaimer.setEditable(false); disclaimer.setBackground(contentPane.getBackground());
        contentPane.add(disclaimer, BorderLayout.NORTH);
        ...
        setLocation((screenDim.width - 415) / 2,(screenDim.height - 260) / 2);
    ...
    private void parseInputs() {
      ...
      Creature attackingCrit = new Creature();
      Creature defendingCrit = new Creature();
      
      Itherian attackingIth = new Itherian();
      Itherian defendingIth = new Itherian();
